...
- `hantokana_app/`：应用代码包
- `test_*.py`：单元测试
- 根目录资源：`icon.ico`、`arrow.svg`、`custom_dict.json`、`dicdir/`、`pykakasi/data/`
...
如果需要自己打包，请直接运行仓库根目录下的 `build.bat`。脚本会创建独立的构建虚拟环境，安装运行与打包依赖，然后使用 `hantokana.spec` 生成程序；如果本机没有配置 Inno Setup，也会自动跳过安装包生成。脚本输出为英文，方便避免 Windows 批处理编码问题。

### 配置与升级策略

应用会把“程序默认值”和“用户实际设置”分开处理：

- `config.json` 只保存用户改过的设置和版本元数据
- 启动时会先合并默认配置，再覆盖用户配置
- 新增配置项时，老用户会自动拿到默认值，不会丢掉原有设置
- 词典文件 `custom_dict.json` 会按词典分组做兼容补全，缺失分组会自动补空
- 安装升级会清理旧安装目录，但不会删除 `%APPDATA%\Hantokana` 下的用户配置和自定义词典

如果以后新增开关或窗口状态，优先加到默认配置里，再在读取配置时做一次归一化；如果是“词典内容”类数据，继续走合并逻辑，只有明确新增的键才覆盖，已有值保持不变。
